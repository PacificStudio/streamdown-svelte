...
describeInNode('remend package custom handlers', () => {
	...
	testInNode('exports parser helpers from the standalone package root', () => {
		expect(parseIncompleteMarkdown('**bold')).toBe('**bold**');

		const parser = new IncompleteMarkdownParser(
			IncompleteMarkdownParser.createDefaultPlugins().filter((plugin) => plugin.name !== 'htmlTags')
		);

		expect(parser.parse('prefix <custom')).toBe('prefix <custom');
	});
