...
| `packages/remend/__tests__/comparison-operators.test.ts`     | `regressions` | `P0`     | `not_started`                | No local regression test covers the `>` and `<` list-item case from upstream issue `#376`.                                                       |
...
| `packages/remend/__tests__/html-tags.test.ts`                | `security`    | `P0`     | `partial_local_coverage`     | Local markdown/MDX tests exist, but the upstream incomplete-tag stripping contract is not encoded.                                               |
...
| `packages/remend/__tests__/setext-heading.test.ts`           | `parser`      | `P0`     | `partial_local_coverage`     | `src/tests/heading.test.ts` covers headings generally, but not the exact setext/incomplete-heading behavior from upstream.                       |
| `packages/remend/__tests__/single-tilde.test.ts`             | `regressions` | `P0`     | `not_started`                | No local regression test covers the upstream single-tilde escape case from issue `#445`.                                                         |
| `packages/remend/__tests__/streaming.test.ts`                | `parser`      | `P0`     | `partial_local_coverage`     | Local `parseIncompleteMarkdown` tests cover recovery behavior, but not the chunk-by-chunk streaming suite.                                       |
