...
| `drift-07` | Root helper/UI exports stay Svelte-first instead of matching the frozen React root export family                                     | Allowed only as approved package-boundary drift for the still-unported helper/UI families such as the animate plugin and code/table UI component exports. Public compatibility exports for icon-context, detect-direction, and URL-policy helpers must stay implemented and tested. |
...
| `drift-09` | Plugin wiring stays on the root `plugins` prop instead of matching the frozen React provider shape                                   | Allowed only as explicit plugin-contract drift. The shared Svelte `plugin-context` compatibility exports must remain public and tested, while any remaining React-specific provider differences stay documented rather than implied to be pending implementation.                   |
