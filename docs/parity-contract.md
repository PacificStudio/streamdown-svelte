...
| `drift-10` | React-specific memo/comparator and deferred-render internals stay unported; Svelte parity is judged on observable DOM behavior       | Allowed only for reference suites whose assertions depend on `React.memo`, custom comparator call patterns, rerender counts, or deferred-render hook timing. The local streaming parser cache now covers latest-document splitting plus stable-block lex reuse, while transient tail updates are intentionally left uncached to avoid unbounded cache growth, so this drift is limited to React-only scheduling internals rather than markdown parse caching. |
