...
| `drift-10` | React-specific memo/comparator and deferred-render internals stay unported; Svelte parity is judged on observable DOM behavior       | Allowed only for reference suites whose assertions depend on `React.memo`, custom comparator call patterns, rerender counts, or deferred-render hook timing. The local streaming parser cache now covers block splitting and per-block lex reuse separately, so this drift is limited to React-only scheduling internals rather than markdown parse caching. |
