...
describeInBrowser('ported streamdown mermaid download controls', () => {
	testInBrowser('toggles the download menu and exposes PNG, SVG, and MMD actions', async () => {
		...
		await vi.waitFor(() => {
			expect(document.querySelector('button[title="Download diagram as PNG"]')).toBeTruthy();
		...
		await vi.waitFor(() => {
			expect(document.querySelector('button[title="Download diagram as PNG"]')).toBeNull();
		});
	});
	...
	testInBrowser('downloads Mermaid source, SVG markup, and PNG blobs', async () => {
		...
		(document.querySelector('button[title="Download diagram as PNG"]') as HTMLButtonElement).click();
