...
describeInBrowser('ported streamdown image and footnote rendering', () => {
	...
	testInBrowser('renders footnote references and definitions through a trailing footnotes section', () => {
		const screen = render(Streamdown, {
			content: [
		...
		const firstRefLink = screen.container.querySelector('sup[data-streamdown-sup] a[href="#footnote-1"]');
		...
		const firstBackref = footnotesSection?.querySelector('a[data-footnote-backref][href="#footnote-ref-1"]');
