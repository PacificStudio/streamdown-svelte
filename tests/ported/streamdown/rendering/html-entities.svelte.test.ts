...
describeInBrowser('ported streamdown html entities', () => {
	testInBrowser('decodes html entities in plain text and link text without decoding code spans', () => {
		...
		expect(screen.container.textContent).toContain('Entities: © 2026 — Streamdown • Built with ♥');
		expect(screen.container.textContent).toContain('& Entity Link');
		expect(screen.container.querySelector('[data-streamdown-link]')?.textContent).toBe('& Entity Link');
